<template>
  <div>
    ...
    <div class="charts-row">
      <div class="card"><div class="card-title">城市 TOP10</div><div ref="barDom" class="bar-box"></div></div>
      <div class="card"><div class="card-title">平台分布</div><canvas id="link-platforms" class="pie-box"></canvas></div>
    </div>
    <div class="charts-row" style="margin-top:14px">
      <div class="card"><div class="card-title">浏览器分布</div><canvas id="link-browsers" class="pie-box"></canvas></div>
      <div class="card"><div class="card-title">省份分布</div><canvas id="link-regions" class="pie-box"></canvas></div>
    </div>
    ...
    <div class="card" style="margin-top:18px">
      ...
      <div class="table-wrap" v-if="records.length">
        ...
        <tbody><tr v-for="r in records" :key="r._id"><td class="mono">{{esc(r.ip)}}</td><td>{{loc(r)}}</td><td>{{esc(r.geoInfo?.isp)}}</td><td>{{plat(r.userAgent)}}</td><td>{{br(r.userAgent)}}</td><td>{{fmt(r.createdAt)}}</td></tr></tbody></table>
      </div>
...
const link = ref({}), cities = ref([]), allVisits = ref([]), records = ref([]);
const page = ref(1), totalPages = ref(1), mapDom = ref(null), barDom = ref(null);
...
let mapInst, barInst, nationalGeo, geoCache = {};
...
function plat(ua){if(!ua)return'未知';if(ua.includes('iPhone')||ua.includes('iPad'))return'iOS';if(ua.includes('Android'))return'Android';if(ua.includes('Windows'))return'Windows';if(ua.includes('Macintosh')||ua.includes('Mac OS'))return'macOS';if(ua.includes('Linux'))return'Linux';return'其他'}
function br(ua){if(!ua)return'未知';if(ua.includes('MicroMessenger'))return'微信';if(ua.includes('Edg/'))return'Edge';if(ua.includes('Chrome/')&&!ua.includes('Edg/'))return'Chrome';if(ua.includes('Safari/')&&!ua.includes('Chrome/'))return'Safari';if(ua.includes('Firefox/'))return'Firefox';return'其他'}

async function load(){
  const id = route.params.id;
  try{
    const [geoRes, regRes, visitRes] = await Promise.all([
    ...
    drawCharts();
    drawPies();
  }catch(e){ console.warn('LinkDetail load error:', e); }
}
...
function renderMap(mapName, geo, zoom){
  const el = mapDom.value; if(!el) return;
  if(mapInst) mapInst.dispose();
  const rawData = cities.value;
  if(!rawData.length) return;
  const gSet = new Set(geo.features.map(f => f.properties.name));
  const m = n => gSet.has(n) ? n : gSet.has(n+'市') ? n+'市' : gSet.has(n+'区') ? n+'区' : gSet.has(n+'县') ? n+'县' : n;
  const md = rawData.map(p => ({ name: m(p.name), value: p.value }));
  const mx = Math.max(...rawData.map(p => p.value), 1);
  ...
  mapInst.setOption({
    ...
    geo:{map:mapName,zoom,center:geo===nationalGeo?[104.5,36]:void 0,roam:true,label:{show:false},emphasis:{label:{color:'#8B6914',fontSize:12,fontWeight:'bold',show:true},itemStyle:{areaColor:'#f5e6c8'}},itemStyle:{areaColor:'#0d1117',borderColor:'#2a3345',borderWidth:geo===nationalGeo?0.5:0.8}},
    series:[{name:'访问',type:'map',map:mapName,geoIndex:0,data:md}],
  });
  // 点击下钻
  const isNation = geo === nationalGeo;
  const cGeo = isNation ? nationalGeo : geo;
  mapInst.off('click');
  mapInst.on('click', p => {
    if(!p.name || !cGeo) return;
    if(isNation){
      const f = nationalGeo.features.find(fe => fe.properties.name === p.name);
      if(f?.id) drillTo(String(f.id).substring(0,2), 'prov');
    } else {
      const sn = p.name.replace(/市$/, '');
      const f = nationalGeo?.features.find(fe => fe.properties.name === p.name || fe.properties.name === sn);
      if(f?.id) drillTo(String(f.id), 'city');
    }
  });
}

function drawCharts(){
  const be = barDom.value; if(!be) return;
  renderMap('link_map', nationalGeo, 1.15);
  // TOP10 柱状图
  const rawData = cities.value;
...
async function drillTo(code, level){
  let url, mk, zoom;
  if(level === 'prov'){ url = `/api/geojson?code=${code}0000`; mk = 'lp_'+code; zoom = 2.5; }
  else { url = `/api/geojson?code=${code}`; mk = 'lc_'+code; zoom = 4; }
  try{
    ...
    drillProv.value = level==='prov' ? (PM[code+'000000']||code) : (geoCache[mk].features[0]?.properties?.name||code);
    renderMap(mk, geoCache[mk], zoom);
...
function drawPies(){
  ...
  const data=allVisits.value;if(!data.length)return;
  function agg(fn){const m={};data.forEach(d=>{const v=fn(d)||'未知';m[v]=(m[v]||0)+1});const t=Object.values(m).reduce((s,v)=>s+v,0)||1;return Object.entries(m).map(([n,c])=>({name:n,pct:Math.round(c/t*100)})).sort((a,b)=>b.pct-a.pct)}
  const cols=['#C9A84C','#1A1A2E','#8B7355','#D4C5C0','#E8D5C4','#8B6914','#A89070'];
  [{id:'link-platforms',data:agg(d=>plat(d.userAgent))},{id:'link-browsers',data:agg(d=>br(d.userAgent))},{id:'link-regions',data:agg(d=>d.geoInfo?.province)}].forEach(c=>{
    const cv=document.getElementById(c.id);if(!cv)return;
    chartInstances.push(new Chart(cv,{type:'doughnut',data:{labels:c.data.map(i=>i.name),datasets:[{data:c.data.map(i=>i.pct),backgroundColor:c.data.map((_,i)=>cols[i%cols.length]),borderColor:'rgba(8,10,15,0.8)',borderWidth:2}]},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{position:'bottom',labels:{color:'#8892a4',padding:16,font:{size:11},usePointStyle:true}}}}}));
  });
}
...
.map-box{width:100%;height:500px}.bar-box{width:100%;height:420px}.pie-box{max-height:240px}
